...
internal class LoginHandlerService(
    ...
    suspend fun initCall(): Screen {
        val httpResponse =
            httpService.post(
                URLBuilder(issuer).apply { path("/flow/api/v1/init") }.build(),
                sessionId,
            )
    ...
    this.forEach { value ->
        when (value) {
            ...
            is Enum<*> -> list.add(JsonPrimitive(value.toString()))
                ...
                "Can't serialize unknown collection type: ${value::class.simpleName}",
            )
        }
    }
    ...
    this.forEach { (key, value) ->
        ...
        when (value) {
            ...
            is Enum<*> -> map[key] = JsonPrimitive(value.toString())
                ...
                "Can't serialize unknown type: ${value::class.simpleName}",
            )
        }
    }
