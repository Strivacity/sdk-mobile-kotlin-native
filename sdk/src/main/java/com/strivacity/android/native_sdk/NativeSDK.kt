...
class NativeSDK
    internal constructor(
        ...
        @Deprecated(
            "Use login with fallbackHandler instead. This call will be removed in future version",
        )
        suspend fun login(
        ...
        suspend fun entry(
            ...
            withContext(dispatchers.IO) {
                ...
                if (response.status == HttpStatusCode.InternalServerError) {
                    ...
                    onError(
                        UnknownError(
                            RuntimeException("Server failed to answer - 500 status code received"),
                        ),
                    )
                    return@withContext
                }
                ...
                if (redirectLocation.isEmpty()) {
                    onError(
                        UnknownError(
                            RuntimeException("Expected to find redirect body but it was not found"),
                        ),
                    )
                    return@withContext
                }
                ...
                if (sessionId == null) {
                    onError(
                        UnknownError(
                            RuntimeException(
                                "Failed to start session: session_id missing or blank",
                            ),
                        ),
                    )
                    return@withContext
        ...
        fun isRedirectExpected(): Boolean = loginController?.isRedirectExpected ?: false
        ...
        suspend fun logout(): Unit =
            withContext(dispatchers.IO) {
                ...
                try {
                    ...
                    if (redirectURI == null ||
                        redirectURI.lowercase() != postLogoutURI.lowercase()
                    ) {
                        logging.warn(
        ...
        private suspend fun continueFlow(
            ...
            if (state != oidcParams.state) {
                ...
                cleanup()
                oidcParams.onError(
                    InvalidCallbackError("State param did not matched expected value"),
                )
                return
            }
        ...
        internal suspend fun refreshTokensIfNeeded(): Boolean =
            tokenRefreshMutex.withLock {
                ...
                logging.debug(
                    "NativeSDK: Access token expired or expiring soon, attempting refresh",
                )
                val refreshToken =
                    session.profile.value
                        ?.tokenResponse
        ...
        companion object {
            ...
            private const val ERROR_DESCRIPTION_KEY: String = "error_description"
        }
    }
